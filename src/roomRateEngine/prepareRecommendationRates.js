 ...
 */
const prepareRecommendationJson = ({
  occupancy,
  roomRatesJson,
    ...
    for (const recommendation of Object.values(RECOMMENDATIONS)) {
      ...
      for (let ratePos = 0; ratePos < recommendation.rates.length; ratePos++) {
        ...
        if (matchedOccupancy && !matchedRateIdForThisOccupancy) {
          ...
          const occupancyMap = recommendationResult.get(occIdx) || new Map();
  ...
  const sortedRecommendations = new Map(
    [...recommendationResult].map(([occIdx, recMap]) => {
      ...
      [...recMap.values()]
        .sort(
          (a, b) => a.finalRateOfRecommendation - b.finalRateOfRecommendation,
        )
        .forEach((rec) => {
 ...
 */
const autoSelectRoomRecommendations = ({
  occupancy,
  roomRatesJson,
  onAutoSelectionDone,
}) => {
